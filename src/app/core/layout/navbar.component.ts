...
@Component({
  ...
  template: `
    <header class="sticky top-0 z-10 border-b border-slate-800 bg-slate-950/95 backdrop-blur">
      <nav class="mx-auto flex h-[72px] max-w-6xl items-center justify-between px-4 sm:px-6">
        <a routerLink="/" class="text-xl font-bold tracking-tight text-violet-400">
          Nightfall <span class="text-slate-100">Stream</span>
        </a>
        <div class="flex items-center gap-1 text-sm font-medium text-slate-300 sm:gap-3">
          <a
            routerLink="/"
          ...
          <a
            ...
            routerLinkActive="text-violet-400"
            class="hidden rounded-lg px-2 py-2 hover:text-violet-400 sm:block"
          >
            My List {{ favorites.ids().length }}
          </a>
          <a routerLink="/profile" routerLinkActive="text-violet-400" class="rounded-lg px-2 py-2 hover:text-violet-400">
            Profile
          </a>
